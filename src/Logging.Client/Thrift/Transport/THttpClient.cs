...
namespace Thrift.Transport
{
    public class THttpClient : TTransport, IDisposable
    {
        ...
        private HttpWebRequest CreateRequest()
        {
            ...
            // connection.Credentials.AddRange(certificates);
            ...
            if (connectTimeout > 0)
            {
                ...
                connection.ContinueTimeout = connectTimeout;
            }
            if (readTimeout > 0)
            {
                //connection.ReadWriteTimeout = readTimeout;
                connection.ContinueTimeout = readTimeout;
            }
#endif
            // Make the request
            connection.ContentType = "application/x-thrift";
            connection.Accept = "application/x-thrift";
            connection.Headers["User-Agent"] = "C#/THttpClient";
